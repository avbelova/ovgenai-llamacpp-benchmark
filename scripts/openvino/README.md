...
### 9. `visualize_performance.py`

**Purpose:** Generate interactive HTML visualizations of benchmark results with device-specific performance charts.

**Usage:**
```bash
python visualize_performance.py <csv_filename>
```

**Example:**
```bash
python visualize_performance.py benchmarking_results_2026-05-05_16-52.csv
```

**What it does:**

1. **Loads and filters benchmark data:**
   - Reads the consolidated CSV file from `parse_benchmark_logs.py`
   - Filters out failed tests and invalid data
   - Converts metrics to numeric format for analysis

2. **Groups results by device:**
   - Separates benchmarks into CPU, GPU, and NPU categories
   - Organizes data by model and framework combinations

3. **Creates interactive visualizations:**
   - Generates dual charts for each device showing:
     - **Time to First Token (TTFT):** Lower is better - measures responsiveness
     - **Token Generation Speed:** Higher is better - measures throughput
   - Groups bars by framework for easy comparison
   - Uses color-coded framework identification

4. **Produces summary statistics:**
   - Calculates mean ± standard deviation for key metrics per device:
     - TTFT (ms)
     - Token Generation (tok/s)
     - Prompt Evaluation (tok/s)
     - Average Token Latency (ms/tok)

**Metrics visualized:**

| Metric | Description | Unit | Preference |
|--------|-------------|------|------------|
| TTFT | Time To First Token - latency before first output | ms | Lower is better |
| TOKEN_GEN | Token generation throughput | tok/s | Higher is better |
| PROMPT_EVAL | Prompt processing throughput | tok/s | Higher is better |
| AVG_TOKEN_LATENCY | Average per-token generation time | ms/tok | Lower is better |

**Framework color scheme:**

The script uses a consistent color palette across all charts to identify frameworks:
- llama.cpp variants (CPU, Vulkan, OpenVINO backends)
- OpenVINO GenAI GGUF reader configurations
- OpenVINO GenAI IR formats (CW, GS32, DEFAULT quantization)
...
**Input:**
- CSV file generated by `parse_benchmark_logs.py`
- Format: 18 columns with benchmark metrics per configuration

**Output:**

1. **HTML file:** `visualize_performance.html`
   - Interactive Plotly charts
   - Hover tooltips with exact values
   - Zoomable and downloadable plots
   - Device-grouped visualizations
...
CPU (45 tests):
  TTFT (ms):              245.67 ± 89.23
  Token Gen (tok/s):      28.45 ± 5.12
  Prompt Eval (tok/s):    125.34 ± 23.45
  Avg Latency (ms/tok):   35.16 ± 6.34

GPU (42 tests):
  TTFT (ms):              123.45 ± 34.56
  Token Gen (tok/s):      67.89 ± 12.34
  ...
================================================================================

✓ Interactive HTML visualization saved: visualize_performance.html
  Open the file in your browser to view interactive charts!
```

**Use cases:**

1. **Quick performance comparison:** Identify fastest framework per device at a glance
2. **Cross-device analysis:** Compare how models perform on CPU vs GPU vs NPU
3. **Framework evaluation:** Determine which backend works best for specific models
4. **Presentation-ready charts:** Export interactive visualizations for reports
5. **Performance regression testing:** Track changes across benchmark runs

**Interpretation tips:**

- **Lower TTFT = Better responsiveness** for interactive applications
- **Higher TOKEN_GEN = Better throughput** for batch processing
- Look for consistent performance across models within a framework
- GPU typically shows higher throughput but may have higher TTFT
- NPU is optimized for efficiency, compare with power consumption data

**Troubleshooting:**

| Issue | Cause | Solution |
|-------|-------|----------|
| "File not found" | Incorrect CSV filename | Check filename matches output from parse script |
| Empty charts | All tests failed or filtered | Review CSV for valid data, check filter criteria |
| Missing frameworks | Benchmarks didn't run | Verify benchmark logs exist for all configurations |
| Import errors | Missing dependencies | Run `pip install pandas plotly` |
...
python visualize_performance.py benchmarking_results_*.csv
```
---
